...
// Only initialize SQLite if NOT running on Vercel / read-only serverless environment
if (typeof window === 'undefined' && !process.env.VERCEL && process.env.NEXT_RUNTIME !== 'edge') {
  try {
    let Database;
    try {
    ...
    if (Database) {
      ...
      try {
        ...
        console.warn('Could not initialize SQLite instance:', e);
      }
    }
  } catch (err) {
    console.warn('SQLite initialization skipped:', err);
  }
}
