...
export async function getNormalizedSnapshotFromSupabase(supabase: SupabaseClient, key: string) {
  ...
  // 1. Pipe Capacities
  const pipeCapacities = (pipeRes.data || []).map((r: any) => ({
    gudang: r.gudang,
  ...
  // 2. Fast Slow
  const fastSlowData = (fsRes.data || []).map((r: any) => ({
    gudang: r.gudang,
  ...
  // 3. Coil Strip
  const coilStripData = (coilRes.data || []).map((r: any) => ({
    gudang: r.gudang,
    area: r.area,
  ...
  // 4. NC Warehouse
  const ncWarehouseData = (ncWhRes.data || []).map((r: any) => ({
    gudang: r.gudang,
  ...
  // 5. NC Items
  const ncItems = (ncItemsRes.data || []).map((r: any) => ({
  ...
  // 7. UNFIFO Items
  const unfifoData = (unfifoRes.data || []).map((r: any) => ({
    gudang: r.gudang,
    kodeMaterial: r.kode_material,
    ukuran: r.ukuran,
    customer: r.customer,
  ...
  // 8. UNFIFO Coil
  const unfifoCoilData = (unfifoCoilRes.data || []).map((r: any) => ({
    gudang: r.gudang,
    kodeMaterial: r.kode_material,
  ...
  // 9. UNFIFO Pipe
  const unfifoPipeData = (unfifoPipeRes.data || []).map((r: any) => ({
    gudang: r.gudang,
    kodeMaterial: r.kode_material,
    ukuran: r.ukuran,
    customer: r.customer,
  ...
  const damagedPackagingData = (damagedRes.data || []).map((r: any) => ({
    id: r.item_id || String(r.id),
    no: r.no_urut,
  ...
  // 12. Stock Opname
  const stoRow = stoRes.data && stoRes.data.length > 0 ? stoRes.data[0] : null;
...
export async function saveNormalizedSnapshotToSupabase(
  ...
  if (data.pipeCapacities !== undefined) {
    ...
    if (data.pipeCapacities.length > 0) {
  ...
  if (data.coilStripData !== undefined) {
    ...
    if (data.coilStripData.length > 0) {
  ...
  if (data.ncWarehouseData !== undefined) {
    ...
    if (data.ncWarehouseData.length > 0) {
  ...
  if (data.unfifoCoilData !== undefined) {
    ...
    if (data.unfifoCoilData.length > 0) {
  ...
  if (data.unfifoPipeData !== undefined) {
    ...
    if (data.unfifoPipeData.length > 0) {
  ...
  if (data.damagedPackagingData !== undefined) {
    ...
    if (data.damagedPackagingData.length > 0) {
  ...
  if (data.ncProgressData !== undefined) {
    ...
    if (data.ncProgressData.length > 0) {
